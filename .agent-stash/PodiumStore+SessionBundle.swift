...
extension PodiumStore {
    ...
    @discardableResult
    public func importSessionRow(_ session: Session) throws -> Int {
        try db.run(
            """
    ...
    @discardableResult
    public func importAgentRow(_ agent: Agent, fallbackSessionId: String) throws -> Int {
        try db.run(
            """
    ...
    @discardableResult
    public func importEventRow(_ event: DashboardEvent, fallbackSessionId: String) throws -> Int {
        try db.run(
            """
    ...
    @discardableResult
    public func importTokenUsageRow(_ usage: TokenUsage, fallbackSessionId: String) throws -> Int {
        try db.run(
            """
    ...
    public func importSessionBundle(_ bundle: SessionExportBundle) throws {
        try db.transaction { _ in
            try self.importSessionRow(bundle.session)
            for agent in bundle.agents { try self.importAgentRow(agent, fallbackSessionId: bundle.session.id) }
            for event in bundle.events { try self.importEventRow(event, fallbackSessionId: bundle.session.id) }
            for usage in bundle.tokenUsage { try self.importTokenUsageRow(usage, fallbackSessionId: bundle.session.id) }
        }
    }
}
