...
| **P6.2a** | Contract E2E: ContractTests harness + fixtures + contract-check.sh + manual browser walk of every web page vs Swift server | P2–P4 (server gated) | 🟨 (adopted from @P 2026-07-05; harness ✅ 29/29 + 6 wire-bug families fixed; left: contract-check.sh + browser walk) |
...
| 2026-07-04 | P6.1 packaging | Sonnet 5 | ⚠️ | macOS FULLY verified: scripts/package-macos.sh ran end-to-end (release build, .app w/ podium-hook+AppIcon+WebClient/dist in Resources, ad-hoc codesign, dist/Podium-1.0.dmg (12MB) — mounted, sandbox-launched w/ fixture HOME/CLAUDE_HOME/DASHBOARD_DATA_DIR + -podium_port NSUserDefaults override; health 200, dashboard served FROM BUNDLE, hooks installed only in fixture; without port override it correctly detected the real Docker on 4820 and rode as client — didn't double-host). WebDistResolver gained bundle-Resources tier: env → /usr/local/share/podium/web → Bundle Resources → dev walk-up; zero EmbeddedServer/main.swift changes needed. .gitignore dist patterns root-anchored after orchestrator warning (git check-ignore verified WebClient/dist stays tracked). run.sh extended (guarded) but NOT run live — would replace the binary under Gaël's running app. **CAVEAT (the ⚠️): Linux build lint-only — docker daemon stalled/unresponsive under machine load; scripts shellcheck-clean + unit hand-reviewed, but the actual swift:6.1 compile, tarball, and systemd install never executed. Close out during/after P6.2 on a settled machine or real Linux box.** NOTE: prod podium container now reports UNHEALTHY (Up 11 days) — predates P6.1, read-only observed. |
...
| 2026-07-05 | P6.2a adopted + contract suite green (interactive Fable session) | Fable 5 | 🟨 | @P lane confirmed dead by Gaël; adopted its untracked WIP ContractTests.swift (complete, well-built — 29 tests/1302 lines). Ran it: 83 failures → triaged into 6 buckets, ALL real product wire bugs except two test-side issues. FIXED: (1) Workflows camelCase family — nested keys (stats.totalSessions, concurrency.aggregateLanes, ConcurrencyLane.avgStart, SubagentEffectivenessItem mixed-casing, etc.) were snake_cased by PodiumJSON; custom `encode(to:)` via the AnyEncodable dictionary pattern on 11 structs in Workflow.swift, shapes verified against types.ts directly. (2) Null-omission — Session/Agent/DashboardEvent/AgentTreeNode now emit explicit JSON null for nullable SQL columns (Node parity; synthesized encode omitted them). (3) /api/settings/info — TranscriptCacheStats reshaped to Node's {size,maxSize,hits,misses,keys} (was {entries}), memory.heapTotal/heapUsed camel fixed. (4) /api/push/vapid-public-key publicKey casing fixed. (5) UpdatesStatusResponse gained required git_repo field (walks up from cwd for .git). (6) SearchHit now encodes per-kind exact Node shapes (session hits omit event keys; event hits carry explicit nulls). Plus: RunHandle + DashboardRun custom encodes (always-present camelCase publicHandle keys / snake+isLive history rows with explicit nulls; DashboardRunWire deleted), POST /api/run now 200 not 201 (Node parity; RunRouterTests updated). TEST-SIDE fixes: /bin/true does not exist on Darwin 25 → /usr/bin/true (misleading 'file doesn't exist' spawn error); PodiumPaths HOME override added so cc-config MCP doesn't read the dev's real ~/.claude.json. ModelTests fixtures corrected where they'd baked in the wrong wire (RunHandle snake keys, memory snake keys, transcript_cache entries). RESULT: ContractTests 29/29, full suite 463/463. REMAINING for P6.2a ✅: scripts/contract-check.sh + manual browser walk of every page. |
| 2026-07-05 | CI root-caused + hardened; P6.2b docs; ROADMAP.md (evening, Fable session) | Fable 5 | ✅ | THE CI HANGS WERE A REAL PRODUCT BUG: RunSpawner ran blocking `waitUntilExit()` in `Task.detached` — parks a cooperative-pool thread per child, pool width = CPU count, so 2–4-core GH runners deadlocked the whole test process mid-suite (macOS froze on the reap test, Linux froze entering RunSpawnerTests at 260/260 passed); a 14-core dev Mac never starves, hence 'only fails on CI'. Fixed with Thread.detachNewThread (RunSpawner.swift). Workflow hardened: timeout-minutes 25 both jobs (no more 6h zombies), concurrency cancel-in-progress, --product-per-invocation (footgun). CI TRUTH POLICY set by Gaël: local-green + runner-red ⇒ GITHUB_ACTIONS-gated XCTSkip w/ documented mechanism — applied to ContractTests health-wait (Linux container networking, same as DiagnosticsRouterTests) and HookClientTests dead-port-latency (>5s connect-fail on runners, both OSes; product safety is the 1s HookClient.requestTimeout, not the test). Run 28751737923: Linux ✅ 18m14s (first green Linux since the contract suite landed), macOS red only on the now-gated HookClient test. P6.2b: README/MIGRATION/CLAUDE.md rewritten (Sonnet draft, Fable-reviewed; old MIGRATION.md was an unrelated WidgetKit stub). NEW: ROADMAP.md at repo root = full post-v1.0 plan w/ copy-paste dispatch prompts (Phase 0 CI+P6.2 close, Phase 1 v1.0 close-out+switchover, Phase 2 v1.1 features ranked, debt ledger, fence block). Global skill /orch-fable created (~/.claude/skills/orch-fable) — Fable-orchestrator operating manual for Opus, since Fable retires shortly. Audit debt: LinuxDesktopNotifier/GitContext/RunBinaryLocator also call waitUntilExit — check pool context (ledger row in ROADMAP Phase 3). |
| 2026-07-05 | FIRST FULLY-GREEN CI (run 28755002211, both jobs) — RunSpawner concurrency saga closed | Fable 5 | ✅ | FOUR stacked real product bugs in RunSpawner, each masking the next, all small-machine-only: (1) waitUntilExit in Task.detached → cooperative-pool deadlock (pool width = nCPU); (2) parent kept child-facing pipe write ends → read side never saw EOF → blocking availableData drain wedged the actor on no-output children; (3) readabilityHandler never detached at EOF → level-triggered EOF re-fires forever → 2 GCD threads spinning per finished run until reap (5 min in prod!) — local suite dropped 18.7s→14.2s when fixed, the storm's CPU bill made visible; (4) the inFlightIO wait documented in attachIO/InFlightIOCounter/LiveRun was LOST from onProcessTerminated — finalize outran in-flight envelope reads (spawning→completed with envelopes queued; likely the true root of the historical 'flaky headless-completion' timeout-bump era). Plus one Linux-only TEST fix: corelibs bridges any 0/1 NSNumber to Bool → objCType=='c' is the only real bool marker (ContractTests classifier). ROADMAP Phase 0.1 ✅. Remaining Phase 0: contract-check.sh (0.2), browser walk (0.3), README close-out (0.4). |
