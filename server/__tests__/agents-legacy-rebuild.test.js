 ...
 * earlier workflow migration and recreates idx_agents_workflow.
 *
 * Regression coverage for an upstream bug: the rebuild's CREATE TABLE
 * agents_new / INSERT INTO agents_new SELECT / index-recreate block was
...
// Same backend fallback chain as db.js — better-sqlite3 has no working build
// on every dev machine (see CLAUDE.md), so fall through to the compat shims.
let Database;
try {
...
before(() => {
  ...
  // Hand-build a pre-workflow legacy DB: a sessions table just rich enough to
  // satisfy the agents FK, and an agents table using the old 3/5-status CHECK
  // (still containing 'idle') with none of the columns added by later
  // migrations (workflow_run_id, workflow_phase, updated_at,
  // awaiting_input_since). This is the exact shape the 'idle' CHECK detector
  // in db.js's rebuild block is looking for.
...
describe("legacy agents_new rebuild (pre-workflow DB)", () => {
  it("loads db.js against the legacy DB without throwing", () => {
    delete require.cache[require.resolve("../db")];
    assert.doesNotThrow(() => require("../db"));
  });

  it("adds workflow_run_id and workflow_phase to agents", () => {
    const { db } = require("../db");
    const cols = db.prepare("PRAGMA table_info(agents)").all();
    const names = cols.map((c) => c.name);
    for (const col of ["workflow_run_id", "workflow_phase"]) {
